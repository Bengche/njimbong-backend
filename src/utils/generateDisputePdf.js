...
// ─── Main export ──────────────────────────────────────────────────────────────
export function generateDisputePdf({
  order,
  buyer,
  seller,
  listing,
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(8.5)
      .text("Confidential — for Fonlok dispute resolution only", ML + 144, 33, {
        lineBreak: false,
      });

    // Generated timestamp top-right
    doc
      .fillColor(C.headerMeta)
      .font("Helvetica")
      .fontSize(7.5)
      .text(generatedAt, ML, 50, {
        width: CW,
        align: "right",
        lineBreak: false,
      });
    ...
    doc
      .fillColor(C.labelBlue)
      .font("Helvetica-Bold")
      .fontSize(7.5)
      .text("BUYER", ML + 12, y + 9, {
        lineBreak: false,
    ...
    doc
      .fillColor(C.labelGreen)
      .font("Helvetica-Bold")
      .fontSize(7.5)
      .text("SELLER", COL2X + 12, y + 9, {
        lineBreak: false,
        characterSpacing: 0.5,
      });
    doc
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(8.5)
      .text(seller.email, COL2X + 12, y + 38, {
        width: HALF - 24,
        lineBreak: false,
      });
    ...
    if (msgCount === 0) {
      ...
      const ntH = doc.heightOfString(noticeText, {
        width: CW - 32,
        lineGap: 3,
      });
      const noticeBoxH = ntH + 36;

      if (y + noticeBoxH > BOTTOM_SAFE) {
        doc.addPage();
        y = 60;
      }
      ...
      doc
        .fillColor(C.labelAmber)
        .font("Helvetica-Bold")
        .fontSize(10)
        .text("No prior chat recorded on this listing", ML + 14, y + 11, {
          lineBreak: false,
        });
      doc
        .fillColor(C.labelAmber)
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        doc.font("Helvetica").fontSize(10);
        const contentH = doc.heightOfString(msg.content, {
          width: CW - 36,
          lineGap: 2.5,
        });
    ...
    for (let p = 0; p < range.count; p++) {
      ...
      doc
        ...
        .font("Helvetica")
        .fontSize(7)
        .text(`Page ${p + 1} of ${range.count}`, ML, fy, {
          width: CW,
          align: "right",
          lineBreak: false,
        });

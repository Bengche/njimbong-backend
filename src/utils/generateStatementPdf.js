...
function statusStyle(status) {
  if (["completed","released"].includes(status)) return { bg: "#f0fdf4", text: "#15803d" };
  if (["refunded"].includes(status))             return { bg: "#eff6ff", text: "#1d4ed8" };
  if (["paid_in_escrow"].includes(status))        return { bg: "#fefce8", text: "#a16207" };
  if (["disputed"].includes(status))             return { bg: "#fef2f2", text: "#b91c1c" };
  if (["failed","cancelled"].includes(status))   return { bg: "#f3f4f6", text: "#4b5563" };
...
export function generateStatementPdf({ transactions, user, from, to }) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(MUTED_LIGHT).font("Helvetica").fontSize(7.5)
       .text(`Period: ${periodLabel}`, ML, y + 27);
    doc.fillColor(MUTED_LIGHT).font("Helvetica").fontSize(7.5)
       .text(
         `Generated: ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}`,
         0, y + 27, { width: W - ML, align: "right" },
       );
    ...
    const summaryItems = [
      ...
      { label: "Transactions", value: String(transactions.length), color: DARK },
    ];

    summaryItems.forEach((item, i) => {
      const cx = ML + colW * i;
      if (i < summaryItems.length - 1) {
        doc.moveTo(cx + colW, y + 12).lineTo(cx + colW, y + BOX_H - 12)
           .strokeColor(BORDER).lineWidth(0.5).stroke();
      }
      doc.fillColor(MUTED).font("Helvetica").fontSize(6.5)
         .text(item.label.toUpperCase(), cx, y + 13, { width: colW, align: "center" });
      doc.fillColor(item.color).font("Helvetica-Bold").fontSize(9)
    ...
    function drawTableHeader() {
      ...
      doc.moveTo(ML, y + ROW_H - 2).lineTo(ML + CW, y + ROW_H - 2)
         .strokeColor("#d1d5db").lineWidth(0.6).stroke();
    ...
    transactions.forEach((tx, i) => {
      ...
      doc.fillColor(MUTED).font("Helvetica").fontSize(7)
         .text(fmtDate(tx.created_at), COL.date.x + 5, ty,
               { width: COL.date.w - 10, lineBreak: false });

      // Description (title line + sub)
      const desc = tx.listing_title || typeLabel(tx.type);
      const sub  = tx.counterparty || "";
      const descY = sub ? ty - 3 : ty;
      doc.fillColor(DARK).font("Helvetica").fontSize(7.5)
         .text(desc, COL.desc.x + 5, descY,
               { width: COL.desc.w - 10, lineBreak: false, ellipsis: true });
      if (sub) {
        doc.fillColor(MUTED_LIGHT).font("Helvetica").fontSize(6)
           .text(sub, COL.desc.x + 5, descY + 10,
                 { width: COL.desc.w - 10, lineBreak: false, ellipsis: true });
      }

      // Reference
      const ref = tx.order_reference || tx.reference || "";
      doc.fillColor(MUTED_LIGHT).font("Helvetica").fontSize(6.5)
         .text(ref || "-", COL.ref.x + 5, ty,
               { width: COL.ref.w - 10, lineBreak: false, ellipsis: true });
      ...
      doc.fillColor(amtCol).font("Helvetica-Bold").fontSize(8)
      ...
      doc.fillColor(st.text).font("Helvetica").fontSize(6.5)
         .text(lab, pillX, pillY + 2.5, { width: labW, align: "center", lineBreak: false });
